 ...
 */
export async function recordResultsEmail({ leadId, email, ok, error = null, consent = false }, { sheetService = null } = {}) {
  ...
  if (!lead) return;
  // Clicking "Email me the list" is the consent, whether or not the send succeeded.
  if (consent) await attempt('db_lead', runId, () => leadsRepo.grantConsent(lead.lead_id));
  if (ok) await attempt('db_export', runId, () => leadsRepo.markEmailSent(lead.lead_id));
  if (sheetService && lead.sheet_row_number) {
    const fields = { resultsEmailed: ok ? 'yes' : 'failed' };
    if (consent) fields.consent = 'yes';
    await attempt('sheet_update', runId, () => sheetService.updateFields(lead.sheet_row_number, fields));
  }
}
